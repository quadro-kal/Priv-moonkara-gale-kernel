...
#ifdef LCT_TP_COMMON
static struct lct_tp_mifunc lct_tp_fts_mifunc = {
#if LCT_TP_INFO_EN
    .tp_info_buff = "[Vendor]unkown,[FW]unkown,[IC]unkown\n",
    .tp_info_cb = lct_fts_get_tpfwver,
#endif

#if LCT_TP_WORK_EN
    .tp_work_cb = lct_fts_tp_work_callback,
#endif

#if LCT_TP_SELFTEST_EN
    .tp_selftest_cb = lct_fts_tp_selftest_callback,
#endif

#if LCT_TP_GRIP_AREA_EN
    .get_screen_angle_cb = NULL,
    .set_screen_angle_cb = lct_fts_tp_set_screen_angle_callback,
#endif

#if LCT_TP_PROXIMITY_EN
    .tp_proximity_cb = lct_fts_tp_proximity_callback,
#endif
};
#endif
...
#ifdef LCT_TP_COMMON
    ret = lct_tp_create_procfs(&lct_tp_fts_mifunc);
    if (ret < 0) {
        FTS_ERROR("lct_tp_create_procfs failed. ret=%d\n", ret);
    }
#endif
