...
int32_t	cts_proximity_en = 0;

int32_t cts_set_proximity_switch(uint8_t prox_switch)
{
    int ret = 0;

    cts_info("prox_switch=%d, touch_lcm_name=%s\n", prox_switch, touch_lcm_name);
	if ((strcmp(touch_lcm_name, "dsi_panel_c3u_42_0f_0d_dsc_vdo")) != 0) {
        cts_info("No lcm, setting disabled!\n");
	    return -1;
    }
...
EXPORT_SYMBOL_GPL(cts_set_proximity_switch);
