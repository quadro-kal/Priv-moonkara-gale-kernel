...
void cts_doubleclick_enable(struct device *dev, int en)
{
...
int cts_suspend(struct chipone_ts_data *cts_data)
{
    ...
    cts_info("Suspend");
    cts_doubleclick_enable(cts_data->device,cts_gesture_flag);
    cts_info("cts_gesture_flag:%d, cts_is_gesture_wakeup_enabled: %d", cts_gesture_flag, 
    cts_is_gesture_wakeup_enabled(&cts_data->cts_dev));
...
#endif
{
    ...
    cts_data->dev_pm_suspend = false;
    init_completion(&cts_data->dev_pm_resume_completion);
    cts_info("init_pm_completion ....");
#endif

    tp_select_proximity = 4;
...
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    cts_data->dev_pm_suspend = true;
    reinit_completion(&cts_data->dev_pm_resume_completion);
...
static int cts_i2c_driver_pm_resume(struct device *dev)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    cts_data->dev_pm_suspend = false;
    complete(&cts_data->dev_pm_resume_completion);
...
//四供兼容//
	printk ("touch_lcm_name=%s\n",touch_lcm_name);
	 if (((strcmp(touch_lcm_name, "dsi_panel_c3u_42_0f_0d_dsc_vdo"))  == 0) )
        {
...
static int __init lct_tp_icnl_is_lockdown_info_detect(char *str)
{
...
__setup("tp_lockdown_info=", lct_tp_icnl_is_lockdown_info_detect);
