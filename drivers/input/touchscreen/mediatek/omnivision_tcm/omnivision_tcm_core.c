...
ovt_tcm_set_func_en(palm_detect, DC_ENABLE_PALM)
#endif
...
#if SUPPORT_PALM_DETECT
EXPORT_SYMBOL(ovt_tcm_set_func_palm_detect_en_state);
#endif
...
static void ovt_ts_usb_plugin_work_func(struct work_struct *work);
DECLARE_WORK(ovt_usb_plugin_work, ovt_ts_usb_plugin_work_func);

void ovt_ts_usb_event_callback(void)
{
	schedule_work(&ovt_usb_plugin_work);
}
...
static void ovt_ts_usb_plugin_work_func(struct work_struct *work)
{
	...
	if (tcm_hcd->in_suspend) {
        printk("Enter %s(%d): tp is suspend,can not be set\n", __func__, __LINE__);
		return;
	}

    printk("Enter %s(%d): usb_plugged_in=%d\n", __func__, __LINE__, g_touchscreen_usb_pulgin.usb_plugged_in);
	ovt_tcm_set_func_charger_connected_en_state(g_touchscreen_usb_pulgin.usb_plugged_in);
	return;
}
#endif
...
int ovt_gesture_switch(struct input_dev *dev, unsigned int type, unsigned int code, int value)
{
	if (type == EV_SYN && code == SYN_CONFIG) {
		if (value == WAKEUP_OFF) {
			ovt_gesture_flag = false;
		} else if (value == WAKEUP_ON) {
			ovt_gesture_flag = true;
		}
	}
    printk("Enter %s(%d):type=%u,code=%u,value=%d, ovt_gesture_flag=%d\n", __func__, __LINE__,
            type, code , value, ovt_gesture_flag);
...
int32_t	ovt_proximity_en = 0;

int32_t ovt_set_proximity_switch(uint8_t prox_switch)
{
    int ret = 0;

    printk("Enter %s(%d): prox_switch=%d, touch_lcm_name=%s\n", __func__, __LINE__, prox_switch, touch_lcm_name);
	if ((strcmp(touch_lcm_name, "dsi_panel_c3u_46_0f_0c_dsc_vdo")) != 0) {
        printk("Enter %s(%d): No lcm, setting disabled!\n", __func__, __LINE__);
	    return -1;
    }
    ...
    else if(0 == prox_switch) {
        set_lcd_reset_gpio_keep_high(false);
        ovt_proximity_en = 0;
        ovt_tcm_set_func_face_detect_en_state(0);
    }
    else {
        ret = -EINVAL;
    }
...
static int ovt_tcm_identify(struct ovt_tcm_hcd *tcm_hcd, bool id)
{
	...
	/*-------add tp_info-------*/
	printk("tcm_hcd->app_info.customer_config_id[15] = %02x",tcm_hcd->app_info.customer_config_id[15]);
	sprintf(tp_info_buf,"[Vendor]omnivision,[FW]0x%02x,[IC]td4160\n",tcm_hcd->app_info.customer_config_id[15]);
	update_lct_tp_info(tp_info_buf, tp_lockdowninfo);
...
#if SUPPORT_PALM_DETECT
	if (tcm_hcd->func_palm_detect_en) {
		retval = ovt_tcm_set_dynamic_config(tcm_hcd, DC_ENABLE_PALM, tcm_hcd->func_palm_detect_en);
		if (retval != 0) {
			LOGE(tcm_hcd->pdev->dev.parent,"Failed to set DC_ENABLE_PALM command\n");
		}
	}
#endif
}
...
static int ovt_tcm_resume(struct device *dev)
{
	...
	if (tcm_hcd->in_hdl_mode) {
            ...
            else
            {
                ...
                printk("Touchscreen Disabled, Can't enable irq.");
            }
        #else
            retval = tcm_hcd->enable_irq(tcm_hcd,true,NULL);
        #endif
...
#if SUPPORT_FACE_DETECT
    LOGN(tcm_hcd->pdev->dev.parent, "ovt_proximity_en = %d\n", ovt_proximity_en);
    if(ovt_proximity_en) {
        ovt_set_proximity_switch(1);
    }
#endif
		...
		unsigned long action, void *data)
{
    ...
    LOGN(tcm_hcd->pdev->dev.parent, "action=%d, *transition=%d\n", action, *transition);
...
int lct_ovt_tp_get_screen_angle_callback(void)
{
	return EPERM;
}

int lct_ovt_tp_set_screen_angle_callback(int angle)
{
    u8 val;
    int ret = -EIO;
    struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
    LOGD(tcm_hcd->pdev->dev.parent,"set angle start!");
    ret = ovt_tcm_set_dynamic_config(tcm_hcd,0x0b,1);
    LOGD(tcm_hcd->pdev->dev.parent,"open grip start!");
    if (ret < 0) {
        LOGE(tcm_hcd->pdev->dev.parent,"open angle failed!");
    }
    ...
    ret = ovt_tcm_set_dynamic_config(tcm_hcd,0xd0,val);
    if (ret < 0) {
        LOGE(tcm_hcd->pdev->dev.parent,"write angle failed!");
    }
    return ret;
}
#endif
...
#if LCT_TP_WORK_EN
int lct_ovt_tp_work_callback(bool en)
{
	int retval;
	struct ovt_tcm_hcd *tcm_hcd;
	printk("call back successfully by xiali");
	...
	if (en) {
        retval = tcm_hcd->enable_irq(tcm_hcd, true, NULL);
        printk("enable irq_by xiali ");
        if (retval < 0) {
            LOGE(tcm_hcd->pdev->dev.parent, "enable interrupt failed by xiali\n");
            goto exit;
        }
	} else  {
        retval = tcm_hcd->enable_irq(tcm_hcd, false, true);
        printk("disable irq by xiali");
        if (retval < 0) {
            LOGE(tcm_hcd->pdev->dev.parent, "Failed to disable interrupt by xiali\n");
            goto exit;
        }
	}
...
int lct_ovt_tp_proximity_callback(bool enFlag)
{
    int ret = 0;

    ret = ovt_set_proximity_switch(enFlag);
    if(ret < 0)
    {
        printk("%s(%d), Set ovt_set_proximity_switch fail!, enFlag=%d\n", __func__, __LINE__, enFlag);
    }
    printk("%s(%d), Set ovt_set_proximity_switch success, flag=%d\n", __func__, __LINE__, enFlag);
...
#ifdef LCT_TP_COMMON
static struct lct_tp_mifunc lct_tp_ovt_mifunc = {
#if LCT_TP_INFO_EN
    .tp_info_buff = "[Vendor]omnivision,[FW]unkown,[IC]td4160\n",
#endif

#if LCT_TP_WORK_EN
    .tp_work_cb = lct_ovt_tp_work_callback,
#endif

#if LCT_TP_SELFTEST_EN
    .tp_selftest_cb = lct_ovt_tp_selftest_callback,
#endif

#if LCT_TP_GRIP_AREA_EN
    .get_screen_angle_cb = NULL,
    .set_screen_angle_cb = lct_ovt_tp_set_screen_angle_callback,
#endif

#if LCT_TP_PROXIMITY_EN
    .tp_proximity_cb = lct_ovt_tp_proximity_callback,
#endif
};
#endif
...
#ifdef LCT_TP_COMMON
    retval = lct_tp_create_procfs(&lct_tp_ovt_mifunc);
    if (retval < 0) {
        LOGE(tcm_hcd->pdev->dev.parent,"lct_tp_create_procfs failed. retval=%d\n", retval);
    }
#endif

#if LCT_TP_DATA_DUMP_EN
    retval = lct_ovt_tp_data_dump_proc_init();
    if (retval < 0) {
        LOGE(tcm_hcd->pdev->dev.parent,"lct_ovt_tp_data_dump_proc_init failed. retval=%d\n", retval);
    }
#endif

#if LCT_TP_USB_PLUGIN
    g_touchscreen_usb_pulgin.event_callback = ovt_ts_usb_event_callback;
#endif
...
#if LCT_TP_DATA_DUMP_EN
    lct_ovt_tp_data_dump_proc_exit();
#endif
...
static int __init ovt_tcm_module_init(void)
{
	...
	if ((strcmp(touch_lcm_name, "dsi_panel_c3u_46_0f_0c_dsc_vdo")) != 0) {
		printk("[TP][%s] lcm not compatible\n",__func__);
		return -ENODEV;
	}

	retval = ovt_tcm_bus_init();
	if (retval < 0) {
		printk("[TP][%s] ovt_tcm_bus_init failed\n",__func__);
		return retval;
	}

	retval = platform_driver_register(&ovt_tcm_driver);
	if (retval < 0) {
		printk("[TP][%s] platform_driver_register failed\n",__func__);
		return retval;
	}

	printk("[TP][%s] finished\n",__func__);
