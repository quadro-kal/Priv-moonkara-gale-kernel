...
#if LCT_TP_INFO_EN
	sprintf(tp_info_buf, "[Vendor]truly,[FW]0x%02x,[IC]nt36528\n", ts->fw_ver);
    NVT_LOG("tp_info: [Vendor]truly,[FW]0x%02x,[IC]nt36528\n", ts->fw_ver);
...
int32_t nvt_check_proximity(uint8_t input_id, uint8_t *data)
{
	...
	if ((input_id == DATA_PROTOCOL) && (func_type == FUNCPAGE_PROXIMITY)) {
		...
		if (proximity_state == PROXIMITY_ON) {
			ps_send_touch_event(proximity_level? 1:0);//0:far
...
static int lct_nvt_tp_get_screen_angle_callback(void)
{
	uint8_t tmp[8] = {0};
	int32_t ret = -EIO;
...
static int lct_nvt_tp_set_screen_angle_callback(int angle)
{
	uint8_t tmp[8];
	int ret = -EIO;
	int retry = 5;
...
int lct_nvt_tp_proximity_callback(bool enFlag)
{
    int ret = 0;

    ret = nvt_set_proximity_switch(enFlag);
    if(ret < 0)
    {
        NVT_ERR("Set nvt_set_proximity_switch fail!, enFlag=%d\n", enFlag);
    }
    NVT_LOG("Set nvt_set_proximity_switch success, flag=%d\n", enFlag);
...
#ifdef LCT_TP_COMMON
static struct lct_tp_mifunc lct_tp_nvt_mifunc = {
#if LCT_TP_INFO_EN
    .tp_info_buff = "[Vendor]unkown,[FW]unkown,[IC]unkown\n",
#endif

#if LCT_TP_WORK_EN
    .tp_work_cb = lct_nvt_tp_work_callback,
#endif

#if LCT_TP_SELFTEST_EN
    .tp_selftest_cb = lct_nvt_tp_selftest_callback,
#endif

#if LCT_TP_GRIP_AREA_EN
    .get_screen_angle_cb = lct_nvt_tp_get_screen_angle_callback,
    .set_screen_angle_cb = lct_nvt_tp_set_screen_angle_callback,
#endif

#if LCT_TP_PROXIMITY_EN
    .tp_proximity_cb = lct_nvt_tp_proximity_callback,
#endif
};
#endif
...
#ifdef LCT_TP_COMMON
    ret = lct_tp_create_procfs(&lct_tp_nvt_mifunc);
	if (ret < 0) {
		NVT_ERR("lct_tp_create_procfs failed. ret=%d\n", ret);
	}
#endif
...
#elif IS_ENABLED(NVT_FB_NOTIFY)
void nvt_ts_esd_resume(int flag)
{
    if(flag == 1)
        nvt_ts_resume(&ts->client->dev);
    else
...
#ifdef LCT_TP_INFO_EN
static int __init lct_tp_is_lockdown_info_detect(char *str)
{
    strlcpy(tp_lockdowninfo, str, sizeof(tp_lockdowninfo));
    strcat(tp_lockdowninfo, "\n");

    return 0;
}
__setup("tp_lockdown_info=", lct_tp_is_lockdown_info_detect);
#endif
...
{
	int32_t ret = 0;
	printk ("touch_lcm_name=%s\n",touch_lcm_name);
	 if (((strcmp(touch_lcm_name, "dsi_panel_c3u_43_02_0a_dsc_vdo"))  == 0) || ((strcmp(touch_lcm_name, "dsi_panel_c3u_43_02_0d_dsc_vdo")) == 0))
        {
                printk ("touch_lcm_name_success=%s\n",touch_lcm_name);
        } else 
        {
                return -1;
        }
